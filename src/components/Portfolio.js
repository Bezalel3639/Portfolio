...
class Portfolio extends Component {
    ...
    updateRate(index, asset) {
        if (asset == 'BTC') {
            ...
            this.getURLRateData(index, endpoints_base + "/v1/Ethereum/ETHRate/");
             ...
             // USD_USD is always 1.00, but keep the syntax as base can differ from USD
             this.getURLRateData(index, endpoints_base + "/v1/FiatSolutions/GetRate/USD/USD");  
        } else if (asset == 'HKD') {
             this.getURLRateData(index, endpoints_base + "/v1/FiatSolutions/GetRate/HKD/USD");  
        } else {
    ...
    updateAmount(index, asset, address) {
        if (asset == 'BTC') {
            ...
            URL = endpoints_base + "/v1/FiatSolutions/GetBalance/malevany%40gmail.com/HKD";
            this.getURLBalanceData(index, URL); 
        } else {
            console.log ("Currency is fiat");
        }
                            ...
                            <tr key={index}>
                                ...
                                {                                    
                                    this.isCrypto(this.state.rows[index].name) ? (
                                        ...
                                        ) : ( this.state.rows[index].amount != 'N/A' ? <td>{this.state.rows[index].amount*this.state.rows[index].rate}</td> : <td>N/A</td> 
                                    )       
                                }
