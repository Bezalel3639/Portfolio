...
class BalanceSandbox extends Component {
    ...
    componentWillMount(){
        this.getMongoDBData(endpoints_base + "/v1/BackOffice/GetSandboxBalancesAssets/" + this.props.investor);     
    }

    callback_balanceUpdate(data) {
        console.log("callback_balanceUpdate> " + data);
        var balance = parseFloat(data.replace(",", "."));
        console.log("callback_balanceUpdate> balance: " + balance);
        const rows = [...this.state.rows];  
        console.log("callback_balanceUpdate> rows: " + rows);
        console.log("callback_balanceUpdate> currentRow: " + this.state.currentRow);
    ...
    populateTable(data) {
        ...
        console.log("populateTable> :", this.state.rows);       
    ...
    updateAmount(index, asset, address) {
        if (asset == 'BTC') {
            URL = endpoints_base + "/v1/Bitcoin/Testnet/Balance/" + address; 
            this.getURLBalanceData(index, URL);
        } else if (asset == 'ETH') {
            URL = endpoints_base + "/v1/Ethereum/Testnet/Balance/" + address;
            this.getURLBalanceData(index, URL);
        } else if (asset == 'WAVES') {
            URL = endpoints_base + "/v1/Waves/Testnet/Balance/" + address;
            this.getURLBalanceData(index, URL); 
        } else if (asset == 'RUB') {
            URL = endpoints_base + "/v1/PayPal/sandbox/GetCustomerBalance/" + this.props.investor + "/" + "RUB";
            this.getURLBalanceData(index, URL); 
            console.log ("updateAmount> Currency: " + asset + ", address: " + address);
        }  else if (asset == 'USD') {
            URL = endpoints_base + "/v1/PayPal/sandbox/GetCustomerBalance/" + this.props.investor + "/" + "USD";
            this.getURLBalanceData(index, URL); 
            console.log ("updateAmount> Currency: " + asset + ", address: " + address);
        }  else if (asset == 'HKD') {
           URL = endpoints_base + "/v1/PayPal/sandbox/GetCustomerBalance/" + this.props.investor + "/" + "HKD";
           this.getURLBalanceData(index, URL); 
           console.log ("updateAmount> Currency: " + asset + ", address: " + address);
        } else {
    ...
    getURLBalanceData(index, URL){
        console.log("getURLBalanceData> index:", index); 
        console.log("getURLBalanceData> URL:", URL); 
        $.ajax({
            url: URL, 
            dataType: 'json',
            contentType: 'json',
    ...
    render() { 
        ...
        return (            
            <div align="center">                
                <h3 align="center">Sandbox Balance: {this.props.investor != '' ? this.props.investor : "Investor is NOT DEFINED"}</h3>
                <table border="1">
                    <thead>
